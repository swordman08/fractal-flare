...
const patternModes = [{
  name: "Particles",
  value: "particles" as const,
  icon: Sparkles
}, {
  name: "Fractals",
  value: "fractals" as const,
  icon: Grid3x3
}, {
  name: "Waves",
  ...
  name: "Ribbon",
  value: "ribbon" as const,
  icon: Waves
}, {
  name: "Strobe",
  value: "strobe" as const,
  icon: Sparkles
}, {
  name: "Pulse",
...
export const ControlPanel = ({
  ...
  return <Card className="fixed top-4 right-4 p-4 bg-card/80 backdrop-blur-lg border-border shadow-lg z-10 animate-fade-in">
      <div className="flex flex-col gap-4">
        {/* Close and Fullscreen Buttons */}
        <div className="flex items-center justify-between gap-2">
          <Button variant="outline" size="sm" onClick={handleFullscreen} className="gap-2" title="Toggle Fullscreen (or press F11)">
            <Maximize className="h-4 w-4" />
            Fullscreen
          </Button>
          <Button variant="ghost" size="sm" onClick={onTogglePanel} className="h-8 w-8 p-0" title="Close Panel">
            <X className="h-4 w-4" />
          </Button>
        </div>
        {/* Background Toggle */}
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={onToggleDarkMode} className="gap-2">
            {isDarkMode ? <>
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2 max-h-64 overflow-y-auto pr-1">
            {colorSchemes.map(scheme => <Button key={scheme.value} variant={colorScheme === scheme.value ? "default" : "outline"} size="sm" onClick={() => onColorSchemeChange(scheme.value)} className="text-xs">
                {scheme.name}
              </Button>)}
          </div>
        </div>
        ...
        <div className="space-y-2">
          <span className="text-slate-50 font-medium text-sm">Galaxy Mode                             </span>
          <Button variant={isHopalongMode ? "default" : "outline"} size="sm" onClick={() => onPatternModeChange(isHopalongMode ? "particles" : "hopalong")} className="w-full gap-2">
        ...
        {/* Pattern Mode */}
        {!isHopalongMode && <div className="space-y-2">
            <span className="text-sm font-medium text-foreground">Pattern:</span>
            <div className="grid grid-cols-3 gap-2">
              {patternModes.map(mode => {
            const Icon = mode.icon;
            return <Button key={mode.value} variant={patternMode === mode.value ? "default" : "outline"} size="sm" onClick={() => onPatternModeChange(mode.value)} className="gap-1 p-2 text-xs" title={mode.name}>
                    {mode.name}
                  </Button>;
          })}
            </div>
            {(patternMode === "fractals" || patternMode === "constellation") && <div className="flex items-start gap-2 text-xs text-yellow-600 dark:text-yellow-400 bg-yellow-500/10 p-2 rounded-md">
        ...
        {patternMode !== "hopalong" && <div className="space-y-2">
        ...
        {isHopalongMode && <div className="space-y-2">
            ...
            <Slider value={[hopalongSpeed]} onValueChange={value => onHopalongSpeedChange(value[0])} min={0.1} max={5} step={0.1} className="w-full" />
          </div>}
        ...
        {!isHopalongMode && <div className="space-y-2">
            ...
            <Slider value={[scale]} onValueChange={value => onScaleChange(value[0])} min={0.5} max={3} step={0.1} className="w-full" />
          </div>}
